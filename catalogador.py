...
API = IQ_Option('SEU USUARIO', 'SUA SENHA')
API.connect()
...
def cataloga(par, dias, prct_call, prct_put, timeframe):
	...
	time_ = time()
	sair = False
	data_teste = ['2020-08-27', '2020-08-20', '2020-08-13', '2020-08-06']
	while sair == False:
		...
		'''
		for x in velas:
			if datetime.fromtimestamp(x['from']).strftime('%Y-%m-%d') in data_teste:
				datas_testadas.append(datetime.fromtimestamp(x['from']).strftime('%Y-%m-%d'))

			if len(datas_testadas) <= len(data_teste):
				x.update({'cor': 'verde' if x['open'] < x['close'] else 'vermelha' if x['open'] > x['close'] else 'doji'})
				data.append(x)
			else:
				sair = True
